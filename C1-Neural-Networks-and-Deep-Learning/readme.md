...
In code, `dw1` is shorthand for `dJ/dw1`. The same naming is used for every `d…` variable in these notes; each one answers "how much does the loss change if I nudge just this one thing?":

- `dz`: nudge the raw score `z = w·x + b` (the value before the sigmoid).
- `dw`: nudge a weight `w`.
- `db`: nudge the bias `b`.

(Strictly, `dz` is the change in the loss `L` of one example, while `dw` and `db` in [Gradient Descent on m Examples](#gradient-descent-on-m-examples) are averaged over all `m` examples, so they are the change in the cost `J`.)

This is why the update `w1 = w1 - alpha * dw1` always moves `w1` in the direction that lowers the cost.
